...
const AllTopic = () => {
  ...
  const { courseSlug } = useParams();
   async function getTopics(){
    const response = await axios.get(`https://lms-backend-1-q2w4.onrender.com/api/topics/get-topic/${courseSlug}`)
   ...
   async function setLive() {
    try {
      const response = await axios.put(
        `https://lms-backend-1-q2w4.onrender.com/api/course/update-course/${status === 'true' ? 'false' : 'true'}/${courseSlug}`
      );
